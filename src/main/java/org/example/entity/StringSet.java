...
public class StringSet {
    ...
    public static String[] findUniqueWords() {
    ...
    String[] kelimeler = text.toLowerCase().trim().split(" ");

        Set<String> benzersizKelimeler = new TreeSet<>();
        for (String kelime : kelimeler) {
            if (!kelime.isEmpty() && kelime.matches("[a-zA-Z]+")) {
                benzersizKelimeler.add(kelime);
            }
        }
        System.out.println("Benzersiz kelime sayısı = " + benzersizKelimeler.size());
        return benzersizKelimeler.toArray(new String[0]);
